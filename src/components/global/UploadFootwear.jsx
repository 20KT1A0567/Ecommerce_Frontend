...
const UploadFootwear = () => {
    ...
    const fetchFootwear = async () => {
        ...
        try {
            ...
            const response = await axios.get("https://demo-deployment2-15-syk7.onrender.com/admin/footwear", {
                headers: {
            ...
            setErrorMessage("Error fetching Footwear. Please try again.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post("https://demo-deployment2-15-syk7.onrender.com/admin/upload/footwear", formData, {
                headers: {
                    "Content-Type": "multipart/form-data",
                    Authorization: `Bearer ${token}`,
                },
            });
            alert("Footwear uploaded successfully!");
            resetForm();
            setIsModalOpen(false);
            fetchFootwear();
        } catch (error) {
            console.error("Error uploading Footwear:", error);
            alert(error.response?.data || "An error occurred while uploading the Footwear.");
        } finally {
    ...
    const handleDelete = async (id) => {
        ...
        const token = localStorage.getItem("token");
        try {
            await axios.delete(`https://demo-deployment2-15-syk7.onrender.com/admin/delete/footwear/${id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
            alert("Footwear deleted successfully!");
            setFootwearList(footwearList.filter((item) => item.id !== id));
        } catch (error) {
            console.error("Error deleting Footwear:", error);
            alert("An error occurred while deleting the Footwear. Please try again.");
        }
    };
    ...
    const handleEdit = async (e) => {
        ...
        try {
            const response = await axios.put(
                `https://demo-deployment2-15-syk7.onrender.com/admin/update/footwear/${selectedFootwear.id}`,
                formData,
                {
                    headers: {
                        "Content-Type": "multipart/form-data",
            ...
            setFootwearList(
                footwearList.map((item) => (item.id === selectedFootwear.id ? response.data : item))
            );
            setIsEditModalOpen(false);
            resetForm();
        } catch (error) {
            console.error("Error updating Footwear:", error);
            alert("An error occurred while updating the Footwear. Please try again.");
        } finally {
        ...
        <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            ...
            {loading ? (
                ...
                <TableContainer component={Paper} sx={{ maxHeight: 600 }}>
                    <Table stickyHeader aria-label="footwear table" size="medium">
                        <TableHead>
                            <TableRow>
            ...
            <Dialog open={isModalOpen} onClose={() => setIsModalOpen(false)} fullWidth maxWidth="sm">
                <DialogTitle>Upload Footwear</DialogTitle>
                <DialogContent>
            ...
            <Dialog open={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} fullWidth maxWidth="sm">
                <DialogTitle>Edit Footwear</DialogTitle>
                <DialogContent>
